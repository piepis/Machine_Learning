...
def findContoursAndDrawBoundingBox(image_rgb):
    ...
    for k in np.linspace(-50, 0, 15):
        binary_niblack = cv2.adaptiveThreshold(gray_image,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,17,k) #输出一个黑白的车牌号
    ...
    image,M = deskew.fastDeskew(image) #输出矫正以后的图片
